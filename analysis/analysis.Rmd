...
Try mixture of gaussians. FIXME
...
```{r}
mean(d$rt > 4000 | d$rt < 500, na.rm=TRUE)
...
pdf("~/Projects/tablet_norming/writeup/figures/accuracy_all.pdf",width=7,height=5)
qplot(age.grp, acc,
      fill=trial.type,
      ymin=acc-acc.cil,
      ymax=acc+acc.cih,
      ...
      stat="identity",
      data=ms) + 
  facet_grid(condition ~ trial.type) + 
  xlab("Age (Years)") + 
  ylab("Accuracy") + 
...
```{r}
ms$trial.type <- factor(ms$trial.type)
levels(ms$trial.type) <- c("Familiar Word","ME Control","ME Inference")
...
Another version where we compute Crohnbach's alpha and Guttman's G6:
...
a <- d %>% 
  filter(trial.type != "NOV-fam", !is.na(rt), condition != "storybook",
         condition != "tablet", age.grp == 4) %>%
  select(subid, age.grp,  trial.num, condition, rt) %>%
  group_by(age.grp, condition, subid) %>%
  arrange(subid, trial.num) %>%
  mutate(trial.order = str_c("t",as.character(1:length(trial.num))), 
...
%>%
  summarise(alpha = alpha(as.data.frame(cbind(t1, t2, t3, t4, t5, t6, t7, t8, 
                                    t9, t10, t11, t12, t13, t14, 
                                    t15, t16)), 
                          na.rm=TRUE, delete=TRUE, 
                          check.keys=FALSE)$total$raw_alpha)
...
b <- with(a, as.data.frame(cbind(t1, t2, t3, t4, t5, t6, t7, t8, 
                                    t9, t10)))
alpha(b, na.rm=TRUE, delete=TRUE, 
      check.keys=FALSE)
